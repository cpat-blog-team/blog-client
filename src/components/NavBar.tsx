...
export default function NavBar() {
	...
	const updateDropDownItems = () => {
		...
		if (scopes.manage_blogs) dropDownItems.push({ text: 'Blog Approval', route: '/blogStatus/approved/Pending' });
	...
	return (
		<Header aria-label="IBM Platform Name" className="nav-box-shadow">
					...
					<HeaderMenu aria-label="Admin Actions" menuLinkName="Admin Actions">
						<span data-testid="nav-bar-admin-actions">
							{dropDownItems.map(({ text, route }, idx) => (
								<HeaderMenuItem key={idx} href={route} data-testid={`Nav Bar ${text} Button`}>
									{text}
								</HeaderMenuItem>
							))}
						</span>
			...
			<HeaderGlobalBar>
				...
				<HeaderGlobalAction aria-label="User" onClick={() => setSideNav(sideNav ? false : true)}>
					{!sideNav && <Menu20 />}
					{sideNav && <Close20 />}
				</HeaderGlobalAction>
			</HeaderGlobalBar>
			...
			<HeaderPanel aria-label="Header Panel" expanded={sideNav}>
				<Switcher aria-label="Switcher Container">
					...
					<SwitcherItem
						...
						onClick={() => redirect(`/myBlogList/username/${name}`)}
						aria-label="Link 2"
					>
						My Blog Posts
					</SwitcherItem>
						...
						<Switcher aria-label="Admin Privileges Switcher">
							...
							{scopes.manage_blogs && (
								<SwitcherItem
									...
									aria-label="Link 6"
									href="/blogStatus/approved/Pending"
								>
									Blog Approval
								</SwitcherItem>
							)}
