...
export default function WriteBlog() {
  ...
  const [
    ...
    setOpenCommunityGuidelinesModal,
  ] = useState(false);
  ...
  const submit = async () => {
    try {
      if (editorMode === "update") {
        ...
        axios.patch(`/api/blogs/${_id}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else {
  ...
  const validateTitle = (value) =>
    value ? setInvalidTitle(false) : setInvalidTitle(true);
  const validateSummary = (value) =>
    value ? setInvalidSummary(false) : setInvalidSummary(true);
  const validateContent = (value) =>
    removeHTMLTags(value) ? setInvalidContent(false) : setInvalidContent(true);
  const validateContentOnBlur = () =>
    content ? setInvalidContent(false) : setInvalidContent(true);
